...
| `scripts/build-source-tokens.ts` | Merge step: figma-raw → tokens.json. Knows Figma's naming conventions (`bg/`, `text/`, `gap/`, composite `Font(...)` strings…). Also attaches `darkValue`s. | ✅ pipeline logic |
| `scripts/build-dark-theme.ts` | Generates the placeholder dark palette; anchors real values extracted from the Figma Themes page. | ✅ heuristics live here |
| `style-dictionary.config.ts` | Build step: three custom output formats (CSS vars incl. `[data-theme="dark"]`, Tailwind theme with `var()` colors, flat JSON). | ✅ output shapes |
| `tailwind.config.ts` | Extends Tailwind's theme from `dist/json/tailwind-tokens.json`. Class names mirror Figma names verbatim. | Rarely |
...
`scripts/build-source-tokens.ts` recognizes Figma prefixes and buckets them:
...
`style-dictionary.config.ts` registers three custom formats:
